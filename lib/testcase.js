...
TestCase.prototype.resolveAnnotations = function(annotations) {
    var config  =  this.config,
        ...
        clob = annotations[annotation.VENUS_CLOB];
    ...
    if ( !clob ) {
        clob = [];
    } else if ( !Array.isArray(clob) ) {
        clob = [clob];
    }

    annotations[annotation.VENUS_CLOB] = clob;

    return annotations;
};

/**
 * Get the path on the filesystem for the temporary folder for this test clobs
...
TestCase.prototype.prepareIncludes = function (annotations) {
    var library = annotations[annotation.VENUS_LIBRARY],
        ...
        clobFiles = this.clobFiles = annotations[annotation.VENUS_CLOB];
    ...
    var clobContents = [],
        clobFilePaths=[].concat(this.getClobFilePaths());

    clobFilePaths.forEach(function(filePath){
        _myself.additionalPathsForWatching.push(filePath);
    });
...
TestCase.prototype.getClobFilePaths = function(){
    var clobFiles=[], _myself=this;
    this.clobFiles.forEach(function(filePath){
        var filename = pathHelper(filePath).file,
            absolutePathForMapping = pathm.resolve(pathHelper(_myself.path.replace(/\\/g, '/')).up().path, filePath).replace(/\\/g, '/');
        if(!fs.existsSync(absolutePathForMapping)) {
            logger.warn("inclusion of file " + absolutePathForMapping + " failed");
            logger.warn("the path was resolved as: " + absolutePathForMapping);
            return;
        } else {
            clobFiles.push(absolutePathForMapping);
        }
    });
    return clobFiles;
}

TestCase.prototype.getClobContents = function(clobFiles){
    var clobContents = [];
    clobFiles.forEach(function(filePath){
        var contents = fs.readFileSync(filePath).toString();
        contents = "['"+contents.replace(/\'/g,"\\'").split("\n").join("'\n,'")+"'].join(\"\\n\")";
        clobContents.push(contents);
    });
    if(clobContents.length>0) {
        return "\nvar TestClobs = ["+clobContents.join(",\n")+"];\n";
    }
}
